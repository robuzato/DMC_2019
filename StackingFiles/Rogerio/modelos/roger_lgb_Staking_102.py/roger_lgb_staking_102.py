...
train = pd.read_csv("G:\\Meu Drive\\LP&D\\DMC_2019_task\\DMC\\Roger\\modelos\\roger_lgb_Staking_102.py\\train.csv", delimiter='|')
test = pd.read_csv("G:\\Meu Drive\\LP&D\\DMC_2019_task\\DMC\\Roger\\modelos\\roger_lgb_Staking_102.py\\test.csv", delimiter='|')
identificador = "102"
seed = 1981
limiar = 0.64
pesoZero = 2
...
arquivos =os.listdir("G:\\Meu Drive\\LP&D\\DMC_2019_task\\DMC\\Modelos Estabilizados\\")

trains = [arq for arq in arquivos if arq.lower().endswith("train.csv")]
tests = [arq for arq in arquivos if arq.lower().endswith("test.csv")]
#%%
for nomes in trains:
    print(nomes)
    metaTrain = pd.read_csv("G:\\Meu Drive\\LP&D\\DMC_2019_task\\DMC\\Modelos Estabilizados\\" +nomes ,delimiter = ",")
    train[nomes[:-10]] = metaTrain['Score']
    
for nomes in tests:
    print(nomes)
    metaTest = pd.read_csv("G:\\Meu Drive\\LP&D\\DMC_2019_task\DMC\Modelos Estabilizados\\"+nomes ,delimiter = ",")
    test[nomes[:-9]] = metaTest['Score']        
...
for i in range(0, 1879, 1):
    ...
    for j in range(0,es):
        ...
        model = train_lgb(d_train, d_valid, val_sets=[d_train, d_valid], n_rounds=200, seedd = seeds)
...
for j in range(0,es):
        ...
        seeds = seed * j + 3^j
        model = train_lgb(d_train,d_train, val_sets=[d_train], n_rounds=300, seedd = seeds)
